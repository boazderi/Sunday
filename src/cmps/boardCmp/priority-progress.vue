<template>
  <section class="priority-progress">
    <div class="bar-container flex align-center">
        <div v-if="progressMap.LOW" class="low"
        :style="{ 'flex-basis': `${progressMap.LOW.percent}%` }"></div>
        <div v-if="progressMap.CRITICAL" class="critical"
        :style="{ 'flex-basis': `${progressMap.CRITICAL.percent}%` }"></div>
        <div v-if="progressMap.EMPTY" class="empty"
        :style="{ 'flex-basis': `${progressMap.EMPTY.percent}%` }"></div>
        <div v-if="progressMap.MEDIUM" class="medium"
        :style="{ 'flex-basis': `${progressMap.MEDIUM.percent}%` }"></div>
        <div v-if="progressMap.HIGH" class="high"
        :style="{ 'flex-basis': `${progressMap.HIGH.percent}%` }"></div>
    </div>
